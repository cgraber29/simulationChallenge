...
# 🎲 Simulation Challenge - Monte Carlo Analysiss
...
grViz(
  ...
  W0 [label = 'Initial Wealth\nW0 = 1000']
  Ct [label = 'Coin Flip t\nCt ~ Bernoulli(0.5)']
  Wt [label = 'Wealth t\nWt = 1.5 * W(t-1) if Ct = 1\nWt = 0.6 * W(t-1) if Ct = 0']
...
```{r}
#| label: simple-sim-r-alt
#| fig-cap: R simulation of coin flip game (alternative approach)
#| echo: true
...
X <- rbinom(n = n_sims, size = 1, prob = 0.5)

# Step 2: Compute winnings (deterministic node)
W <- ifelse(X == 1, 100, -100)
...
grViz(
  "digraph timeseries_balance {
  graph [layout = dot, rankdir = LR]
  node [shape = rectangle, style = filled, fillcolor = aliceblue, fontname = Helvetica]

  B0 [label = 'Initial Balance\nB0 = 1000']
  Ct [label = 'Coin Flip t\nCt ~ Bernoulli(0.5)']
  Bt [label = 'Balance t\nBt = Bt-1 + 100 if Ct = 1\nBt = Bt-1 - 100 if Ct = 0']
...
```{r}
#| label: timeseries-sim-r-alt
#| fig-cap: R time-series simulation of account balance (alternative approach)
#| echo: true
...
```{r}
#| label: distribution-sim-r-alt
#| fig-cap: R probability distribution of final balances (alternative approach)
#| echo: true
...
#| echo: true
initial_wealth <- 1000
ev <- 0.5 * 1.5 * initial_wealth + 0.5 * 0.6 * initial_wealth
cat("Expected value after 1 flip: $", format(ev, big.mark = ",", digits = 0), "\n", sep = "")
```
...
```{r}
#| label: single-sim-investment
#| fig-cap: Single simulated account balance from age 25 to 55 (Original Game)
#| echo: true
...
start_age <- 25
end_age <- 55
n_flips <- end_age - start_age  # 30 years
initial_wealth <- 1000.0

simulate_multiplicative_path <- function(initial_wealth, n_flips, p_head = 0.5) {
  wealth <- initial_wealth
  ages <- start_age
  balances <- wealth
  
  for (i in 1:n_flips) {
    head <- rbinom(1, 1, p_head)
    wealth <- wealth * (ifelse(head == 1, 1.5, 0.6))
    ages <- c(ages, start_age + i)
    balances <- c(balances, wealth)
  }
  
  return(tibble(age = ages, balance = balances))
}

single_path <- simulate_multiplicative_path(initial_wealth, n_flips)

ggplot(single_path, aes(x = age, y = balance)) +
  geom_line(color = "teal", linewidth = 1.2) +
  geom_point(color = "darkblue", size = 2) +
  geom_hline(yintercept = 1000, color = "gray", linetype = "dashed", linewidth = 1) +
  labs(
    title = "Account Balance Over Time — Single Simulation (Original Game)",
    x = "Age",
    y = "Account Balance ($)",
    caption = "Dashed line shows initial $1,000"
  ) +
  scale_y_continuous(labels = scales::dollar_format()) +
  theme_minimal() +
  theme(
...
cat("Final balance at age 55: $", format(single_path$balance[nrow(single_path)], big.mark = ",", digits = 2), "\n", sep = "")
```
...
```{r}
#| label: hundred-sims-distribution
#| fig-cap: Distribution of final balances at age 55 across 100 simulations (Original Game)
#| echo: true
...
start_age <- 25
end_age <- 55
n_flips <- end_age - start_age
initial_wealth <- 1000.0
n_sims <- 100

simulate_final_balance <- function(initial_wealth, n_flips, p_head = 0.5) {
  wealth <- initial_wealth
  for (i in 1:n_flips) {
    head <- rbinom(1, 1, p_head)
    wealth <- wealth * (ifelse(head == 1, 1.5, 0.6))
...
final_balances <- replicate(n_sims, simulate_final_balance(initial_wealth, n_flips))
dist_df <- tibble(sim = 1:n_sims, final_balance = final_balances)

ggplot(dist_df, aes(x = final_balance)) +
  geom_histogram(bins = 20, fill = "plum", alpha = 0.85, color = "black") +
  geom_vline(xintercept = initial_wealth, color = "red", linestyle = "dashed", linewidth = 2) +
  labs(
    title = "Final Balance Distribution at Age 55 — 100 Sims (Original Game)",
    x = "Final Balance ($)",
    y = "Frequency",
    caption = "Red line shows initial $1,000"
  ) +
  scale_x_continuous(labels = scales::dollar_format()) +
  theme_minimal() +
  theme(
...
cat("Summary stats — mean, median, min, max, P(> $1,000):\n")
mean_val <- mean(dist_df$final_balance)
median_val <- median(dist_df$final_balance)
prob_above_initial <- mean(dist_df$final_balance > initial_wealth)
cat("Mean: $", format(mean_val, big.mark = ",", digits = 2), "\n", sep = "")
cat("Median: $", format(median_val, big.mark = ",", digits = 2), "\n", sep = "")
cat("Min: $", format(min(dist_df$final_balance), big.mark = ",", digits = 2), "\n", sep = "")
cat("Max: $", format(max(dist_df$final_balance), big.mark = ",", digits = 2), "\n", sep = "")
cat("P(final > $1,000): ", round(prob_above_initial, 3), "\n", sep = "")
```
