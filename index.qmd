...
print(f"Expected value after 1 flip: ${ev:,.0f}")
```
...
def simulate_multiplicative_path(initial_wealth: float, n_flips: int, p_head: float = 0.5) -> pd.DataFrame:
    wealth = initial_wealth
    ages = [start_age]
    balances = [wealth]
    for _ in range(n_flips):
        head = np.random.binomial(1, p_head)
        wealth = wealth * (1.5 if head == 1 else 0.6)
...
ax.plot(single_path["age"], single_path["balance"], color="teal", linewidth=2, marker="o", markersize=4)
ax.axhline(1000, color="gray", linestyle="--", linewidth=1, label="Initial $1,000")
ax.set_title("Account Balance Over Time — Single Simulation (Original Game)", fontsize=13, fontweight="bold")
ax.set_xlabel("Age")
...
print("Final balance at age 55:", f"${single_path['balance'].iloc[-1]:,.2f}")
```
...
def simulate_final_balance(initial_wealth: float, n_flips: int, p_head: float = 0.5) -> float:
    wealth = initial_wealth
    for _ in range(n_flips):
        head = np.random.binomial(1, p_head)
        wealth = wealth * (1.5 if head == 1 else 0.6)
...
final_balances = [simulate_final_balance(initial_wealth, n_flips) for _ in range(n_sims)]
dist_df = pd.DataFrame({"sim": range(1, n_sims + 1), "final_balance": final_balances})

fig, ax = plt.subplots(figsize=(9, 5.5))
ax.hist(dist_df["final_balance"], bins=20, color="plum", alpha=0.85, edgecolor="black")
ax.axvline(initial_wealth, color="red", linestyle="--", linewidth=2, label="Initial $1,000")
ax.set_title("Final Balance Distribution at Age 55 — 100 Sims (Original Game)", fontsize=13, fontweight="bold")
...
print("Summary stats — mean, median, min, max, P(> $1,000):")
mean_val = dist_df["final_balance"].mean()
median_val = dist_df["final_balance"].median()
prob_above_initial = (dist_df["final_balance"] > initial_wealth).mean()
print(f"Mean: ${mean_val:,.2f}")
print(f"Median: ${median_val:,.2f}")
print(f"Min: ${dist_df['final_balance'].min():,.2f}")
print(f"Max: ${dist_df['final_balance'].max():,.2f}")
print(f"P(final > $1,000): {prob_above_initial:.3f}")
```
